...
>
> **The underlying blocker named below is still real and still unresolved.** No ADR-002 pilot category/market rule pack exists. Three inputs the engine needs — the prohibited-category/counterfeit denylist, the destination market, and the price/margin thresholds — ship as clearly labelled, versioned placeholders (`candidate_evaluations.policy_version`), following the same pattern as the shortlist's existing `PLACEHOLDER_MARKET_CODE`, not as an invented business/legal ruling. Every decision the engine currently produces is provisional until a real pilot pack replaces them. See spec [[cj-candidate-to-sals3-product-draft-implementation-spec#Labelled placeholders, not approved policy]].
...
### 2026-08-07 — Shopify-style per-seller CJ connections, Supplier Apps, and AliExpress integration
- **What:** ADR-008's curated Supplier Apps, seller-entered/owned CJ API credentials with a credential vault and per-seller token refresh, AliExpress or other additional supplier providers, and multi-seller production tenancy for the catalog/evaluation pipeline.
- **Why parked:** Explicitly deferred by Bogs on 2026-08-07 when scoping the automated candidate-evaluation pipeline — "ihiwalay muna natin: ngayon ayusin ang current Candidate Explorer, mamaya ang Shopify-style CJ connection." The pipeline continues on the single global `CJ_API_KEY` and the `dev-user`/`seller-001` placeholder context for now.
- **Unblock condition:** Bogs/AJ prioritize this as a separate task. Do not build any part of it speculatively while working on catalog/evaluation features.
