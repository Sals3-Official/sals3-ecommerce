...
- Workspace: `E:\sals3-ecommerce` (merged code + vault repo, confirmed 2026-08-04). `develop` at commit `232a2b9` as of 2026-08-06 — PR #21 (PDP/cart/guest header), #24 (CJ rate-limit cap), and #25 (deals/for-you decoupling) all merged. PR #26 (title/imageAlt truncation fix) and `sals3-portal` PRs #2 (single-product lookup) and #3 (`totalPages` self-correction) open as of 2026-08-06.
...
- [ ] Product detail page (`/p/[id]`) loads real product data, images, price, and related products; an invalid or missing id returns a real 404. **Known regression as of 2026-08-06 (PR #24, accepted stopgap):** the client-side search that finds a product by slug is capped at 2 pages/section to avoid hammering CJ's rate limit, so most real products currently 404 on their own detail page — pending `sals3-portal` PR #2 (single-product lookup) merging. Reviews/description/brand/warranty sections were removed entirely (PR #21) since the real backend has no such fields — not a regression, an honest cut. Agent-verified live in the browser 2026-08-06 (not yet owner-observed — this box means the owner saw it personally, per this checklist's own definition above).
