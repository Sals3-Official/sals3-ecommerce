...
- **Two current queue-state defects are publication blockers.** A normal `TEMPORARILY_INELIGIBLE` decision is saved without `nextRetryAt`, so `NO_STOCK`/`NO_SHIPPING_ROUTE` rows do not perform the automatic retry promised by the portal. A queued row whose connection becomes disconnected/revoked is saved as `EVALUATION_FAILED` below the Exception Queue's maximum-attempt threshold with no retry time, making it invisible to every sourcing queue. Also, supplier snapshots and evaluation rows overwrite the previous version; audit events retain a checksum/decision event but not enough linked historical evidence and complete findings to reproduce every old decision. Fix retry/recovery state invariants and append-only linked evidence/decision history before catalog publication.
- **Disconnect/reconnect fallback approved, not implemented (2026-08-10).** An intentional **Supplier Apps -> Disconnect** is an expected pause—even during development—not a technical failure. Stop ingestion/evidence calls and place affected work in **Blocked / Rejected -> Temporarily unavailable** with stable reason `SUPPLIER_CONNECTION_DISCONNECTED` and recovery trigger `ON_CONNECTION_RESTORED`; do not increment technical attempts or poll while disconnected. **Reconnect and resume evaluation** verifies credentials, audits `CONNECTION_RESTORED`, and performs a bounded requeue through **Evaluating** before any row can return to **Ready**. Preserve the last completed evidence/decision as history while effective listing/publication eligibility is false. Future live offers pause future sales; accepted orders remain immutable.
...
- **CJ evidence and lean catalog controls calibrated in ADR-013, not implemented.** Preserve `cjInventory`, `factoryInventory`, `totalInventory`, and `verifiedWarehouse`; do not hard-code factory-backed stock as either clean pass or permanent block. Rename the current warehouse-stock check so it cannot imply a destination freight route. Start with persistent category/time scan checkpoints and split only when a partition reaches CJ's documented 6,000-record cap. Subscribe imported/live products rather than the raw candidate pool, use simple reconciliation, and treat points exhaustion/API inactivity suspension as recoverable connection health. Phase 1 keeps Sals3/CJ IDs, one market-wide return policy, direct Product Catalogue reads, and truthful trend V0; GTIN/channel feeds, recall automation, sample-inspection software, external-index repair, advanced trends, and complex product-specific returns are parked.
- **AJ product filtering/automation/stock-sync Q&A consolidated (2026-08-10), approved design not implemented.** [[sals3-session-2026-08-10-part21-aj-product-filtering-automation-and-stock-sync]] records the complete discussion from **All Supplier Products** through **Ready**, exact filter settings and decision precedence, queue/connection recovery, seller/supplier media, trend/Google/parked boundaries, and the technical stock-webhook path. Core automation stays in Sals3 TypeScript/PostgreSQL: keep the existing protected scheduler plus DB leases for the pilot, consider Vercel Pro Cron + Vercel Queues only after beta/cost/reliability review, and use n8n only for peripheral alerts/reports/reminders. CJ stock callbacks must be verified/deduplicated, update the exact variant through re-evaluation, use reconciliation and checkout validation, and never silently rewrite accepted orders.
