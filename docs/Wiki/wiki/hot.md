...
- **Confirmed 2026-08-05:** [[sals3-skills]] now has 19 entries — 5 from the landing-page session, 4 from the API/carousel session, 3 from the footer/pagination session, 4 from the machine-endpoints session, and 3 from the home-page SEO/GEO/AEO session (entries 17–19: `generateMetadata` vs `layout.tsx`, `WebSite` `SearchAction` as a forward-looking signal, and Prettier failing `format:check` after multi-chunk edits). See that note for the full list.
...
- **Confirmed 2026-08-05 — sals3-portal: Sals3 fixture catalogue removed, CJdropshipping is now the only product source.** Owner (AJ) chose full deletion. Removed from `sals3-portal`: the in-memory fixture store (12 demo products), `src/lib/products/*`, `src/services/{products,product-store}.ts`, all Sals3 CRUD routes (`/products/new`, `/products/[id]`, `/[id]/edit`, `/import`, `/export`), their components, the `ProductSourceTabs` source switcher, and the fixture e2e spec. `/products` now renders the CJ supplier view directly; old `?source=cj` links still resolve (param ignored). CJ stack, storefront API (`/api/storefront/*`), and `sals3-ecommerce` untouched — smoke-tested 200 with bearer token, 401 without. Auth module kept whole (only `product:read` exercised now; rest retained as future surface). Safety snapshot commit `310a7bb` on branch `chore/pre-catalogue-removal-snapshot` preserves the pre-deletion source (it was untracked); work sits uncommitted on `feat/products-cj-only`. Verified: portal `npm run verify` all green (lint, format, typecheck, build, 48 unit tests, 7 e2e) + `npm audit --audit-level=high` 0 vulnerabilities. Note: 6 e2e tests in the deleted `e2e/products.spec.ts` were already failing before this change (pre-existing, recorded in the snapshot commit message).
...
- See [[sals3-skills]] for all 19 entries.
