...
CJ stock synchronization uses verified/deduplicated product/variant/stock callbacks for selected imports, live products, and accepted-order protection subjects—not the raw **All Supplier Products** pool. A callback appends exact variant/origin evidence, re-evaluates the affected offer, and updates Product Catalogue availability only after the database decision commits. Scheduled reconciliation repairs missed/delayed events, while checkout performs the final exact stock/orderability/cost/freight validation. One zero-stock variant disables only that variant; all variants at zero pauses future checkout without deleting the Product. Stock return never overrides another current blocker, and accepted orders never receive a silent substitution.

The pilot runs core automation inside Sals3 TypeScript/PostgreSQL through the protected scheduler and bounded database leases/retry. A future Vercel Pro Cron/Queues delivery layer is conditional on beta/cost/reliability review and never replaces the Sals3 evidence, tenant authorization, audit, or Exception Queue. n8n is limited to peripheral alerts, reports, reminders, and later back-office integrations.
