...
1. **Persistent coverage per connection.** Store immutable-cutoff scan cycles and hierarchical category/time/price partitions with leases, checksums, counts, failures, and completion proof. CJ discovery uses only legacy `GET /api2.0/v1/product/list`; Product List V2's documented `6000` maximum does not apply. Split a dense partition adaptively by time and then price. If an atomic bucket requires pagination, require two identical complete PID-set checksums and a unique count equal to the reported total. Any non-converging bucket remains `PROVIDER_COVERAGE_UNRESOLVED` and blocks cycle completion.
2. **Continuous cycles plus freshness lanes.** A private managed queue advances bounded discovery and evaluation work without a cron schedule. A fixed cycle cutoff prevents a moving catalogue from invalidating in-progress coverage; products created afterward enter the next cycle. Selected/live/order-linked products receive webhook priority and daily reconciliation, qualified unselected products refresh within 72 hours, and other operational products refresh within 30 days. No lane may permanently starve deep catalogue coverage.
...
10. **Bounded resumable workers.** Ingestion and evaluation keep per-invocation time, supplier-request/points, and row budgets. Work persists its partition/lease state before the runtime deadline and resumes through an at-least-once queue message. Duplicate deliveries and overlapping Start/Resume calls are idempotent and cannot spend duplicate supplier calls for the same lease. Points exhaustion and CJ's documented zero-transaction inactivity suspension are recoverable connection-health states with delayed retry/reactivation and bounded requeue; they are not permanent product decisions.
11. **Observable coverage.** Measure pages/cursors covered, source products seen, candidates created/requeued/unchanged, products not yet queued and why, oldest queue age, full-scan duration, evidence age, policy-version lag, retries/dead letters, supplier points, and per-connection errors. Alert on a stalled checkpoint, growing queue age, incomplete full scan, or a decision still active on an obsolete policy version.
    - Record and monitor queue-chain heartbeat separately from a successful HTTP response. Production must not report healthy discovery when the database is absent, the durable chain has stalled, or the webhook has auto-closed.
