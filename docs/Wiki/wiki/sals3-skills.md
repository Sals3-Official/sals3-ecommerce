...
### 52. `gh pr checks` can omit a failing GitHub Actions run entirely — confirm CI ran by listing runs for the branch

**Confirmed:** 2026-08-07, reviewing why the repo's own `Verify` workflow appeared absent from `sals3-portal#6`.

**Incident:** `gh pr checks 6` listed only `Vercel` and `Vercel Preview Comments`, both passing. That was reported to Bogs as "the repo's own verify workflow does not seem to run on PRs" — a wrong conclusion. `gh run list --branch <branch>` showed a `Verify` run **had** triggered on the same head SHA and had **failed**. The workflow's `on.pull_request.branches: [develop, main]` was configured correctly all along. The failure was infrastructure, not code: `The job was not acquired by Runner of type hosted even after multiple attempts` after 15m1s — GitHub never assigned a hosted runner. Cross-checking `gh run list` on the sibling repo showed a successful run ten minutes later, which is what established it as a transient blip rather than an org-wide billing or capacity problem.

**Lesson:** `gh pr checks` reflects the commit-status/check-run surface, which a workflow run can be missing from — so its output is not evidence about whether CI ran. To answer "did the gate run, and what did it say," use `gh run list --branch <branch>` (add `--json headSha` to confirm it matches the PR head) and then `gh run view <id>`. Separately, read a CI failure's annotation before assuming it is yours: a runner-acquisition message is a queueing failure, and comparing against a recent run in a sibling repo of the same org distinguishes transient from systemic in one command.

**Where applied:** Confirmed the `Verify` workflow triggers correctly on PRs, corrected the earlier claim to Bogs, and let the next push produce a fresh run on the final commit rather than re-running a superseded one.
